...
export class ErrorMatrixPA {

    constructor(){
        this.content = document.getElementById('content-error-matrix-pa');
        this.info = document.getElementById('info-error-matrix-pa');
        this.infoErrorMatrixGlobal = document.createElement('div');
        this.infoErrorMatrixGlobal.id = 'info-error-matrix-global-pa';
        this.infoErrorMatrixFiltered = document.createElement('div');
        this.infoErrorMatrixFiltered.id = 'info-error-matrix-filtered-pa';
    }
    ...
    createConfusionMatrix(lyr, dataLyr){
        ...
        var confusionMatrix = document.getElementById('confusion-matrix-global-pa');
        
        if ( !confusionMatrix ) {
            confusionMatrix = document.createElement('div');
            confusionMatrix.id = 'confusion-matrix-global-pa';
        ...
        this.confusionMatrixHigh = Highcharts.chart('confusion-matrix-global-pa', {
            chart: {
                type: 'heatmap',
                plotBorderWidth: 0,
    ...
    computeDataToErrorMatrix(yCategories, xCategories, dataArea, colors){
        ...
        for (let horizontal = 0; horizontal < lenI; horizontal++) {
            for (let vertical = 0; vertical < lenJ ; vertical++) {
                ...
                count -= 1;
                totalArea[vertical] += value;
            }
        }

        dataErrorMatrix[0].value = parseFloat((dataErrorMatrix[0].value / totalArea[0] * 100).toFixed(2));
        dataErrorMatrix[1].value = parseFloat((dataErrorMatrix[1].value / totalArea[1] * 100).toFixed(2));
        dataErrorMatrix[2].value = parseFloat((dataErrorMatrix[2].value / totalArea[0] * 100).toFixed(2));
    ...
    createConfusionMatrixFiltered(dataLyr, featsUnselect, polygonFilter){
        ...
        var confusionMatrix = document.getElementById('confusion-matrix-filtered-pa');

        if ( !confusionMatrix ) {
            confusionMatrix = document.createElement('div');
            confusionMatrix.id = 'confusion-matrix-filtered-pa';
            confusionMatrix.style.width = '100%';
            confusionMatrix.style.marginBottom = '5px';
            confusionMatrix.style.marginTop = '5px';
        ...
        this.confusionMatrixFiltered = Highcharts.chart('confusion-matrix-filtered-pa', {
            chart: {
                type: 'heatmap',
                plotBorderWidth: 1,
                borderColor: 'rgba(255,255,255,0)',
